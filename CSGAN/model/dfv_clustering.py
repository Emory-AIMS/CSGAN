...
from KMeans_clustering_derived_fv import KMeans_clustering_derived_feature_vec
import pandas as pd
import time
...
# for GeoLife derived feature vector
number_clusters = 7
num_clusters = 7
## -----------------------
# for GeoLife (08-12)
...
if __name__ == '__main__':
    # report_gpu()
    ## -----------------------
    # saving directory for current time out of k times
    current_time = 0

    # for Geolife derived feature vector
    save_dir = 'result_GeoLife_dfv/time_{}'.format(current_time)
    ...
    oracle_samples = oracle_samples[1:,]
    print(oracle_samples[0])
    print(oracle_samples[1])
    ...
    # for KMeans for derived feature vector
    print("Start KMeans Clustering for derived feature vector...")
    # change oracle_samples from a tensor to a numpy array
    real_trajectories = pd.read_csv("../../data/GeoLife/GeoLife_processed_6_20_2008.csv")
    real_trajectories = real_trajectories.iloc[1:]
    print(real_trajectories.iloc[0])
    # for GeoLife
    bbox = (39.85, 40.0, 116.25, 116.5)
    horizontal_n = 225
    vertical_n = 225
    ...
    # vertical_n = 500
    # time_res = 0.25
    KM_df = KMeans_clustering_derived_feature_vec(real_trajectories, num_clusters, bbox, horizontal_n, vertical_n,
                                                      time_res)
    labels, centroids, inertia = KM_df.clustering()
    # get the cluster_ids of the real trajectories
    cluster_ids = labels
    # for i in range(real_trajectories.shape[0]):
    #     cluster_ids.append(KM_df.get_cluster_id(real_trajectories[i]))
    ...
    train_discriminator(KM_df, discriminator, discriminator_optimizer, oracle_samples, cluster_ids,
                        generator, oracle, Discriminator_pretrain_steps, device, verbose=True)
    ...
    for epoch in tqdm(range(ADV_train_epochs)):
        ...
        oracle.load_state_dict(prev_state_dict)
        train_discriminator(KM_df, discriminator, discriminator_optimizer, oracle_samples, cluster_ids, generator,
                            oracle, Discriminator_train_steps, device, verbose=True)
